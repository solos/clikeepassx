...
class KDB4File(KDBFile):
    ...
    def _read_header(self, stream):
        ...
        while True:
            ...
            if length > 0:
                data = stream_unpack(
                    stream, None, length, '{}s'.format(length))
    ...
    def _decrypt(self, stream):
        ...
        data = aes_cbc_decrypt(
            stream.read(), self.master_key, self.header.EncryptionIV)
        data = unpad(data)
    ...
    def _encrypt(self):
        ...
        self.out_buffer = aes_cbc_encrypt(
            data, self.master_key, self.header.EncryptionIV)
...
class KDBXmlExtension:
    ...
    def unprotect(self):
        """
        Find all elements with a 'Protected=True' attribute and replace the
         text with an unprotected value in the XML element tree. The original
         text is set as 'ProtectedValue' attribute and the 'Protected'
         attribute is set to 'False'. The 'ProtectPassword' element in the
         'Meta' section is also set to 'False'.
        """
        self._reset_salsa()
    ...
    def protect(self):
        ...
        is set to 'True'. The 'ProtectPassword' element in the 'Meta' section
        is also set to 'True'.
